...
interface DatePickerProps {
  ...
  send_data_to_parent:(pickup_month:string,end_month:string,pickup_date:string,dropoff_date:string,pickup_time:string,dropoff_time:string,actual_pickup_date:Date,actual_dropoff_date:Date)=>void
...
//comment out
  // useEffect(() => {
  //   if (startDate && endDate && pickTime && dropTime && pickup_time_touched && droff_time_touched) {
  //     const pickup_month = startDate.toLocaleString('default', { month: 'long' });
  ...
  //     send_data_to_parent(pickup_month, end_month, pickup_date, dropoff_date, pickTime, dropTime,startDate,endDate);
  //   }
  // }, [startDate, endDate, pickTime, dropTime, send_data_to_parent]);
...
//comment out ends
  ...
  if (
    ...
    // Clone dates to avoid mutating original startDate and endDate
    ...
    // Send the new datetime objects to parent
    send_data_to_parent(
      pickup_month,
      end_month,
      pickup_date,
...
}, [startDate, endDate, pickTime, dropTime, pickup_time_touched, droff_time_touched, send_data_to_parent]);
...
//   const handleDateClick = (date: Date) => {
//   // 1️⃣ If disablePickup is true, we already have startDate,
//   //    so just treat every click as a drop-off selection.
//   if (disablePickup) {
...
//   // 2️⃣ Otherwise, original two-step logic
//   if (!startDate || (startDate && endDate)) {
//     setStartDate(date);
//     setEndDate(null);
//     setPickTime(isSameDay(date, today) ? nowTimeStr : "10:00");
...
const handleDateClick = (date: Date) => {
  // ——————————————————————
  // Mode: service-started → disablePickup===true
  // ——————————————————————
  if (disablePickup) {
    // if there’s no seeded startDate, bail
    if (!startDate) return;

    // must pick a drop-off ≥ pick-up
    if (date < startDate) {
      setError("Drop-off must be same or after pick-up.");
      setTimeout(() => setError(""), 3000);
      return;
    }

    // no booked days in between
    const blocked = disabledDates.some(d => {
      const dd = parseISO(d);
      return dd >= startDate! && dd <= date;
    });
    if (blocked) {
      setError("Selected range includes a booked date.");
      setTimeout(() => setError(""), 3000);
    ...
    // **always** set endDate to the clicked date
    setEndDate(date);
    setError("");
    onDateChange({
      startDate: format(startDate, "yyyy-MM-dd"),
  ...
  if (!startDate || (startDate && endDate)) {
    ...
    const includesBooked = disabledDates.some(d => {
      const dd = parseISO(d);
      return dd >= startDate && dd <= date;
    });
    if (includesBooked) {
